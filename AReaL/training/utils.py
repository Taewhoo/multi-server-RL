...
class RayWorker:
    ...
    def configure(self, cfg: Any, expr_config: Any):
        ...
        self.logger.info(f"[DEBUG] Configuring {self.worker_type} worker {idx}...")
        self.worker._configure(cfg)
        self.logger.info(f"[DEBUG] Configuring {self.worker_type} worker {idx}... Done.")

    def run_sync(self):
        self.logger.info(f"[DEBUG] Running {self.worker_type} lazy initialization...")
        self.worker._poll()
        self.logger.info(f"[DEBUG] Running {self.worker_type} lazy initialization... Done.")
        while self.worker.status != WorkerServerStatus.PAUSED:
            self.worker._poll()
        self.logger.info(f"[DEBUG] {self.worker_type} worker completed and paused.")

    async def run_async(self):
        self.logger.info(f"[DEBUG] Running {self.worker_type} async lazy initialization...")
        await self.worker._poll_async()
        self.logger.info(f"[DEBUG] Running {self.worker_type} async lazy initialization... Done.")
...
def _run_experiment(exp_cfg, expr_name, trial_name):
    ...
    try:
        # Configure workers
        logger.info(f"[DEBUG] Starting worker configuration for {len(all_workers)} worker types...")
        configure_jobs = []
        for worker_type in all_workers:
            logger.info(f"[DEBUG] Configuring {worker_type} workers...")
            ...
            configure_jobs += jobs
            logger.info(f"[DEBUG] {worker_type} configuration jobs submitted ({len(jobs)} jobs)")

        logger.info(f"[DEBUG] Waiting for all worker configurations to complete ({len(configure_jobs)} total jobs)...")
        ray.get(configure_jobs)
        logger.info(f"[DEBUG] All worker configurations completed successfully!")

        # Run workers
        logger.info(f"[DEBUG] Starting worker execution...")
        run_jobs = []
        for worker_type in all_workers:
            logger.info(f"[DEBUG] Starting {worker_type} workers...")
            workers = all_workers[worker_type]
            ...
            run_jobs += jobs
            logger.info(f"[DEBUG] {worker_type} workers started ({len(jobs)} jobs)")

        logger.info(f"[DEBUG] Waiting for all workers to complete ({len(run_jobs)} total jobs)...")
        ray.get(run_jobs)
        logger.info(f"[DEBUG] All workers completed successfully!")
    finally:
        logger.info(f"[DEBUG] Shutting down Ray...")
        ray.shutdown()
        logger.info(f"[DEBUG] Ray shutdown complete")
