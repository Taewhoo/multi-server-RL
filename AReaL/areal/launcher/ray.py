...
class RayLauncher:
    ...
    def submit_array(
        ...
        # Skip placement group for single-node setups to avoid allocation issues
        if nodes == 1:
        ...
        if amend_torch_dist_env:
            if placement_group is not None:
                ...
                # For single-node setups, use localhost
        ...
        for i in range(count):
            ...
            future = self.submit(
                ...
                env_vars=_env_vars,
                placement_group=placement_group,
                bundle_index=node_id if placement_group is not None else -1,
                kwargs=kwargs,
            )
...
def ray_main():
    ...
    # Set Ray temp directory to avoid /tmp disk space issues
    # Use short path to avoid Unix socket length limit (107 bytes)
    os.environ.setdefault('RAY_TMPDIR', '/raid/taewhoo/ray_tmp')
    ray.init()
    ...
    if allocation_mode.type_ == AllocationType.DECOUPLED_SGLANG:
        ...
        n_sglang_nodes = max(1, allocation_mode.gen_world_size // n_gpus_per_node)
    ...
    trainer_n_nodes = max(1, n_nodes - n_sglang_nodes)
    trainer_entry_point = sys.argv[1]
    
    # Fix: Use allocation mode to determine trainer processes, not n_gpus_per_node
    alloc_mode = AllocationMode.from_str(config.allocation_mode)
    n_trainer_processes = alloc_mode.train_world_size
    ...
    if not config.server_only:
        ...
        launcher.submit_array(
            ...
            func_name=DEFAULT_MAIN_FUNC_NAME,
            count=alloc_mode.train_world_size,
