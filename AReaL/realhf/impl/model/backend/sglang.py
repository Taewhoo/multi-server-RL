...
class SGLangGenerationEngine(PipelinableEngine):
    ...
    def __init__(
        ...
        logger = logging.getLogger("SGLangEngine", "debug")
        
        logger.info(f"[DEBUG] SGLangGenerationEngine __init__ starting for TP rank {constants.tensor_parallel_rank()}")
        
        if constants.tensor_parallel_rank() != 0:
            logger.info(f"[DEBUG] SGLangGenerationEngine: Non-zero TP rank {constants.tensor_parallel_rank()}, waiting for barrier...")
            dist.barrier(group=constants.tensor_parallel_cpu_group())
            logger.info(f"[DEBUG] SGLangGenerationEngine: TP rank {constants.tensor_parallel_rank()} barrier complete, exiting early")
            return
            
        logger.info(f"[DEBUG] SGLangGenerationEngine: TP rank 0, starting server process...")
        ...
        logger.info(f"[DEBUG] SGLangGenerationEngine: Server process started with PID {self.server_proc.pid}")

        self.base_url = f"http://{server_args_dict['host']}:{server_args_dict['port']}"
        logger.info(f"[DEBUG] SGLangGenerationEngine: Base URL set to {self.base_url}")
        ...
        logger.info(f"[DEBUG] SGLangGenerationEngine: Starting wait_server() call...")
        self.wait_server()
        logger.info(f"[DEBUG] SGLangGenerationEngine: wait_server() completed successfully")

        if server_args_dict["enable_metrics"]:
            logger.info(f"[DEBUG] SGLangGenerationEngine: Setting up metrics...")
        ...
        logger.info(f"[DEBUG] SGLangGenerationEngine: Starting weight/cache offload...")
    ...
    def wait_server(self):
        ...
        logger.info(f"[DEBUG] SGLang wait_server: Starting to wait for server at {self.base_url}")
        success = False
        for i in range(SGLANG_INIT_TIMEOUT):
            time.sleep(1)
            logger.info(f"[DEBUG] SGLang wait_server: Attempt {i+1}/{SGLANG_INIT_TIMEOUT} to contact server")
            try:
                res = requests.get(
                    self.base_url + "/get_model_info", timeout=5, headers={}
                )
                assert res.status_code == 200, f"{res=}, {res.text=}"
                logger.info(f"[DEBUG] SGLang wait_server: Server responded successfully on attempt {i+1}")
                success = True
                break
            except (AssertionError, requests.exceptions.RequestException) as e:
                last_traceback = get_exception_traceback()
                logger.info(f"[DEBUG] SGLang wait_server: Attempt {i+1} failed: {type(e).__name__}: {str(e)}")
                pass
        if not success:
            logger.error(f"Initialization failed. warmup error: {last_traceback}")
            kill_process_tree(os.getpid())
            return
        logger.info(f"[DEBUG] SGLang wait_server: Server startup completed successfully")
...
class SGLangGenerationBackend(ModelBackend, SGLangConfig):
    ...
    def _initialize(self, model: Model, spec: FinetuneSpec) -> Model:
        from realhf.base import logging
        logger = logging.getLogger("SGLangBackend", "debug")
        
        logger.info(f"[DEBUG] SGLang backend initialization starting for model {constants.model_name()}")
        ...
        logger.info(f"[DEBUG] SGLang: Preparing configuration for model {constants.model_name()}")
        additional_args = dataclasses.asdict(self)
        additional_args.pop("hybrid_train")
        additional_args["random_seed"] = seeding.get_seed()

        logger.info(f"[DEBUG] SGLang: Finding free ports for model {constants.model_name()}")
        ...
        logger.info(f"[DEBUG] SGLang: Using ports {api_server_port} (API) and {dist_port} (dist) for model {constants.model_name()}")
        ...
        logger.info(f"[DEBUG] SGLang: Creating SGLangGenerationEngine for model {constants.model_name()}...")
        model.module = SGLangGenerationEngine(
            server_args_dict,
            hybrid_train=self.hybrid_train,
        )
        logger.info(f"[DEBUG] SGLang: SGLangGenerationEngine created successfully for model {constants.model_name()}")
