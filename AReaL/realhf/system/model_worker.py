...
class ModelWorker(worker_base.Worker):
    ...
    def __lazy_setup(self):
        ...
        logger.info(f"[DEBUG] Model worker {self.__worker_index}: Starting model and backend initialization...")
        recover_ckpt_paths = []
        for s in self.config.shards:
            logger.info(f"[DEBUG] Model worker {self.__worker_index}: Processing shard {s.id.model_name}")
            with constants.model_scope(s.id.model_name):
                ...
                logger.info(f"[DEBUG] Model worker {self.__worker_index}: Creating model {s.id.model_name}...")
                self.__models[s.id.model_name] = model = model_api.make_model(
                    s.model, name=s.id.model_name, device=self.__device
                )
                logger.info(f"[DEBUG] Model worker {self.__worker_index}: Model {s.id.model_name} created successfully")
                ...
                if s.should_instantiate:
                    if isinstance(model.module, ReaLModel):
                        logger.info(f"[DEBUG] Model worker {self.__worker_index}: Instantiating model {s.id.model_name}...")
                        model.module.instantiate()
                        logger.info(f"[DEBUG] Model worker {self.__worker_index}: Model {s.id.model_name} instantiated")
                    self.__model_is_handle[s.id.model_name] = False
                else:
                    self.__model_is_handle[s.id.model_name] = True
                
                logger.info(f"[DEBUG] Model worker {self.__worker_index}: Creating backend for {s.id.model_name}...")
                self.__backends[s.id.model_name] = model_api.make_backend(s.backend)
                logger.info(f"[DEBUG] Model worker {self.__worker_index}: Backend created for {s.id.model_name}")
                ...
                logger.info(f"[DEBUG] Model worker {self.__worker_index}: Creating interface for {s.id.model_name}...")
                self.__interfaces[s.id.model_name] = model_api.make_interface(
                    interface_impl[0]
                )
                logger.info(f"[DEBUG] Model worker {self.__worker_index}: Interface created for {s.id.model_name}")

                if s.eval_dataset is not None:
                    logger.info(f"[DEBUG] Model worker {self.__worker_index}: Creating eval dataset for {s.id.model_name}...")
                    ...
                    logger.info(f"[DEBUG] Model worker {self.__worker_index}: Eval dataset created for {s.id.model_name}")
                else:
                    eval_dataloader = None
                self.__eval_dataloaders[s.id.model_name] = eval_dataloader

        logger.info(f"[DEBUG] Model worker {self.__worker_index}: All models initialized, starting checkpoint path synchronization...")
        ...
        logger.info(f"[DEBUG] Model worker {self.__worker_index}: Starting barrier for checkpoint synchronization...")
        dist.barrier()
        logger.info(f"[DEBUG] Model worker {self.__worker_index}: Checkpoint synchronization barrier complete")

        logger.info(f"[DEBUG] Model worker {self.__worker_index}: Setting up request/reply queues...")
        ...
        logger.info(f"[DEBUG] Model worker {self.__worker_index}: Model worker initialization COMPLETE!")
